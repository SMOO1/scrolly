...
smoothing_factor = 0.5  # Smoothing factor for cursor movement (0-1)
prev_x, prev_y = pyautogui.position()  # Store previous cursor position
...
control_zone_size = 0.1  #size of the dead zone (20% of frame width/height)
...
def process_command(command):
    """Process voice command with Gemini and execute"""
    if not command:
        return
        
    #first check for search commands
...
def perform_search(command):
    """Handle search commands by focusing search bar and typing query"""
    try:
        #extract the search query from the command
        if command.lower().startswith("search for"):
            query = command[10:].strip()
        elif command.lower().startswith("search "):
            query = command[7:].strip()  #added this line to handle "search [query]"
        ...
        #focus the search bar (Command+L on Mac, Ctrl+L on Windows)
        if pyautogui.KEYBOARD_KEYS.get('command'):
            pyautogui.hotkey('command', 'l')  # mac
        else:
            pyautogui.hotkey('ctrl', 'l')     #windows/Linux
        
        #wait a bit for search bar to focus
        time.sleep(0.8)  # Increased delay for better reliability
        
        #clear any existing text in the search bar
        pyautogui.hotkey('command', 'a') if pyautogui.KEYBOARD_KEYS.get('command') else pyautogui.hotkey('ctrl', 'a')
        ...
        print(f"Error performing search: {str(e)}")  #error reporting
...
def head_movement_control():
    """Handle head movement cursor control"""
    ...
    while running:
        ...
        if len(faces) > 0:
            ...
            if (face_center_x < zone_x1 or face_center_x > zone_x2 or 
                ...
                #non linear scaling (accelerate mouse movement the farther away dot is)
                scale_factor = 1.5
                ...
                target_x = int(prev_x + (smooth_x * screen_width * 0.8))  # control sensitivity
                ...
                #apply smoothing between current and target position
...
def main():
    ...
    #start head movement control in main thread
    try:
